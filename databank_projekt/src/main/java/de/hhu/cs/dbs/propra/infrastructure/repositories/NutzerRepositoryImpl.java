...
public class NutzerRepositoryImpl implements NutzerRepository {
    ...
    public Optional<Nutzer> findByName(String name) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            String sql = "SELECT ROWID, * FROM Nutzer n WHERE n.Benutzername LIKE ?;"; // TODO: Dem Benutzer 'name' die entsprechenden Berechtigungen geben (siehe enum Role), um diese per RolesAllowed-Annotation zu nutzen. Ein Ergebnistupel besteht aus E-Mail-Adresse bzw. Benutzernamen und Berechtigung.
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    public void save(Nutzer nutzer){
        try (Connection connection = dataSource.getConnection()) {
            String sql = "INSERT INTO Nutzer VALUES (?,?,?,5);";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
